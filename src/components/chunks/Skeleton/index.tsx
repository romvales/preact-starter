...
export const Skeleton: FunctionComponent<SkeletonProps> = props => {
  ...
  if (props.animated) className += props.animationClass ? ` ${props.animationClass}` : ' skel-defanim'
  if (props.circular) className += ' rounded-full'

  return (
    <div aria-busy={true} className={`skel dark:bg-slate-800 bg-gray-300 ${className}`.trim()} style={skelStyle}>
      {props.children}
    </div>
  )
}
